                ...
                <div class="form-group">
                  ...
                  <select class="form-control" id="vehicle_make_id" name="vehicle_make_id" required>
                    @foreach($makes AS $make)
                      <option value="{{ $make->id }}" {{ $data->makeId == $make->id?'selected':'' }}>
                        {{ $make->title }}
                      </option>
                    @endforeach
                  </select>
                ...
                <div class="form-group">
                  ...
                  <select class="form-control" id="vehicle_model_id" name="vehicle_model_id" required>
                    @foreach($models AS $model)
                      <option value="{{ $model->id }}" {{ $data->vehicle_model_id == $model->id?'selected':'' }}>
                        {{ $model->title }}
                      </option>
                    @endforeach
                  </select>
            ...
            <div class="row">
              ...
              <div class="col-sm-10">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" id="client_name" name="client_name" value="{{ $data->client_name }}" required>
                </div>
                <div class="form-group">
                  <label class="form-check-label" for="client_phone">Phone</label>
                  <input type="tel" class="form-control" id="client_phone" name="client_phone" placeholder="1234567890" pattern="[0-9]{10}" value="{{ $data->client_phone }}" required>
                </div>
                <div class="form-group">
                  <label class="form-check-label" for="client_email">Email-ID</label>
                  <input type="email" class="form-control" name="client_email" value="{{ $data->client_email }}" required>
                </div>
              </div>
            </div>
          </fieldset>
